document.addEventListener('DOMContentLoaded', () => {
    ...
    productCards.forEach(card => {
        ...
        card.addEventListener('click', (e) => {
            ...
            let url = new URL('product_details.html', window.location.href);
            if (titleEl) url.searchParams.set('name', titleEl.innerText);
            if (priceEl) url.searchParams.set('price', priceEl.innerText.replace('₹', ''));
            if (imgEl) url.searchParams.set('img', imgEl.src);
    ...
    window.updateCartCount = () => {
        ...
        const cartBadges = document.querySelectorAll('[aria-label="Cart"] .badge');
    ...
    // Run on load to set initial counter
    updateCartCount();

    // Redirect to cart page when clicking the Cart icon
    const headerCartBtns = document.querySelectorAll('[aria-label="Cart"]');
